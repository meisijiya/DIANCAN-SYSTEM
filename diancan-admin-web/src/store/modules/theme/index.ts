...
export const useThemeStore = defineStore(SetupStoreId.Theme, () => {
  ...
  const naiveThemeOverrides: Ref<App.Theme.NaiveUIThemeOverride | undefined> = ref(
    localStg.get('themeNaiveOverrides') || undefined
  );
  ...
  scope.run(() => {
    ...
    // persist theme settings in real time so refresh/close keeps user choice
    watch(
      settings,
      () => {
        cacheThemeSettings();
    ...
    // persist preset-specific naive ui overrides, otherwise refresh/logout will fallback to base theme only
    watch(
      naiveThemeOverrides,
      val => {
        if (val) {
          localStg.set('themeNaiveOverrides', val);
          return;
        }

        localStg.remove('themeNaiveOverrides');
      },
