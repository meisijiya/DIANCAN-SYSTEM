...
export const useRouteStore = defineStore(SetupStoreId.Route, () => {
  ...
  async function initDynamicAuthRoute() {
    ...
    if (!error) {
      ...
      const sanitizedRoutes = sanitizeDynamicRoutes(routes);
      addAuthRoutes(sanitizedRoutes);
  ...
  function sanitizeDynamicRoutes(routes: ElegantConstRoute[]): ElegantConstRoute[] {
    const cleanRoute = (route: ElegantConstRoute): ElegantConstRoute | null => {
      const hasComponent = !!route.component;
      const rawChildren = route.children || [];
      const children = rawChildren
        .map(child => cleanRoute(child as ElegantConstRoute))
        .filter(Boolean) as ElegantConstRoute[];

      // 过滤掉无 component 且无有效 children 的空节点，避免 vue-router warn
      if (!hasComponent && children.length === 0) {
        return null;
      }
