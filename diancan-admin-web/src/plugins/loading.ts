...
export function setupLoading() {
  const themeColor = overrideThemeSettings.themeColor || localStg.get('themeColor') || themeSettings.themeColor;
  ...
  const bgBase = darkMode ? '#04070c' : '#eef6ff';
  const bgLayer = darkMode ? '#0b111b' : '#ffffff';
  const titleColor = darkMode ? 'rgba(241,246,255,0.96)' : '#173358';
  const subtitleColor = darkMode ? 'rgba(206,216,236,0.72)' : 'rgba(21,44,76,0.62)';
  ...
  const cssVars = `${primaryColor}; ${svgCssVars}; --loading-bg-base: ${bgBase}; --loading-bg-layer: ${bgLayer}; --loading-title-color: ${titleColor}; --loading-subtitle-color: ${subtitleColor}`;
...
function getLogoSvg() {
  const logoSvg = `<svg
      ...
      >
        ...
        <circle cx="74" cy="26" r="10" fill="rgba(255,255,255,0.15)" />
        <path
        ...
        <path
          ...
          stroke-linecap="round"
        />
        <path d="M69 24 79 34" fill="none" stroke="rgba(255,255,255,0.94)" stroke-width="3.2" stroke-linecap="round" />
        <path d="M65 20 75 30" fill="none" stroke="rgba(255,255,255,0.72)" stroke-width="2" stroke-linecap="round" />
        <defs>
